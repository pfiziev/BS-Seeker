...
if __name__ == '__main__':
	...
	parser.add_option("-o", "--output", type="string", dest="outfilename",help="The name of output file [BS_SEEKER_SE_ZYMO_OUTPUT.txt]", metavar="OUTFILE")
	#----------------------------------------------------------------
	...
	logoutfile='log_BS_Seeker_SE_'+out_filename
	...
	for chr in d:
		...
		logoutf.write("ref seq: %s %12d bp"%(chr,len(genome_seqs[chr]))+"\n")
	d.close()
	...
	for read_file in my_files:
		...
		if asktag=="Y":
			...
			nn=0
			for ali in [(FW_C2T_uniq_lst,FW_C2T_U),(RC_G2A_uniq_lst,RC_G2A_U),(FW_G2A_uniq_lst,FW_G2A_U),(RC_C2T_uniq_lst,RC_C2T_U)]:
				nn+=1
				...
				for xx in ali_unique_lst:
					...
					if nn==1: 							# +FW mapped to + strand:
						FR="+FW"
						mapped_location=mapped_location+1
		...
		if asktag=="N":
			...
			nn=0
			for ali in [(FW_C2T_uniq_lst,FW_C2T_U),(RC_C2T_uniq_lst,RC_C2T_U)]:
				nn+=1
				ali_unique_lst=ali[0]
				ali_dic=ali[1]
				mapped_chr0=""
	...
	logoutf.write("Number of raw reads: %d "%(all_raw_reads)+"\n")
	if all_raw_reads >0:
		logoutf.write("Number of reads having adapter removed: %d "%(all_trimed)+"\n")
		logoutf.write("Number of unique-hits reads for post-filtering: %d"%(all_mapped)+"\n")		
		if asktag=="Y":
